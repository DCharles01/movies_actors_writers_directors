...
def drop_all_tables(conn, cursor):
    ...
    table_names = ['movies', 'actors', 'writers', 'directors', 'movie_actors', 'movie_writers', 'movie_directors']

    # check if using test table
    dbname = conn.get_dsn_parameters().get('dbname')

    if 'test' in dbname:
        for i,table in enumerate(table_names):
            table_names[i] = table + '_test'
...
def drop_create_tables(conn, cursor):
    ...
    if 'test' in dbname:
        with open('../db/migrations/create_test_tables.sql', 'r') as sql_file:
            sql_query = sql_file.read()

        cursor.execute(sql_query)
        conn.commit()
    else:
        raise 'Warning: attempting to delete prod table'
